...
class GPUStatCollection(object):
    ...
    @staticmethod
    def _test_amd_smi_init(write_fd, debug=False):
        import os, sys
        if not debug:
        ...
        os.close(write_fd)
        from clearml_agent.helper.gpu.pyrsmi import smi_get_kernel_version, smi_initialize
        smi_initialize()
    ...
    def _get_amd_driver_version():
        ...
        try:
            ...
            # we use Process for our subprocess to make sure it works with
            # compiled version of the agent (no sys.executable)
            # noinspection PyBroadException
            try:
                import os
                import multiprocessing as mp
                # Create a pipe and run as a separate process
                read_fd, write_fd = os.pipe()

                # Start the process, passing the write end
                p = mp.Process(target=GPUStatCollection._test_amd_smi_init, args=(write_fd, False, ))
                p.start()
